package com.liuzengyu.examples.es.demo.action.del;
...
public class DeleteIndex {
    ...
    public void doDeleteIndex() {
        ...
        request.timeout(TimeValue.timeValueMinutes(1000));
        //连接master节点的超时时间(使用TimeValue方式)
        request.masterNodeTimeout(TimeValue.timeValueMinutes(1000));
        try {
            // 调用delete
            AcknowledgedResponse response = client.indices().delete(request, RequestOptions.DEFAULT);
            System.err.printf("isAcknowledged:%s", response.isAcknowledged());
            log.info("isAcknowledged:%s" + response.isAcknowledged());
        } catch (IOException e) {
